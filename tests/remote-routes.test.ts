...
describe('GET /remote', () => {
  ...
  it('returns the SPA shell for an authenticated operator', async () => {
    ...
    // All seven nav pages exist in the shell (Lower Thirds was removed — that's
    // an operator-level task now, out of scope for Remote).
    for (const page of ['page-slides', 'page-stills', 'page-packages', 'page-urls', 'page-timer', 'page-prompter', 'page-settings']) {
