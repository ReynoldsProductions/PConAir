...
export function createGscCompatRouter(store: StateStore, windowManager?: SlidesWindowManager): Router {
  const router = Router();
  ...
  // Notes scroll/zoom — dispatch to window manager.
  router.post('/scroll-notes-up', (_req: Request, res: Response) => {
    windowManager?.scrollNotesUp();
    res.json({ success: true });
  });
  router.post('/scroll-notes-down', (_req: Request, res: Response) => {
    windowManager?.scrollNotesDown();
    res.json({ success: true });
  });
  router.post('/zoom-in-notes', (_req: Request, res: Response) => {
    windowManager?.zoomInNotes();
    res.json({ success: true });
  });
  router.post('/zoom-out-notes', (_req: Request, res: Response) => {
