...
export function validateManifest(raw: unknown): { ok: true; manifest: PackageManifest } | { ok: false; error: string } {
  ...
  if (m.stateSchema !== undefined) {
    ...
    // top-level keys are engine-owned (spec 15's `_transport`, spec 19's
    // `_meta`, this spec's `_data`). A package cannot declare one — if spec 15
    // has already added this check, this is a no-op duplicate of the same rule.
    for (const key of Object.keys(m.stateSchema as Record<string, unknown>)) {
      if (key.startsWith('_')) {
        return { ok: false, error: `stateSchema key '${key}' must not start with '_' (reserved for the engine)` };
      }
    }
  }
