...
(function () {
  ...
  function controlPanel(el, opts) {
    ...
    function buildPreview(root, doc) {
      ...
      try {
        /* P.preview's real signature is (el, opts) -- it opens its own
           internal control-role connection scoped to opts.packageId/
           renderId rather than reusing this panel's client (spec 17 s3.3).
           Passing this panel's client as a second positional argument here
           used to silently become preview()'s `opts` (so packageId read as
           undefined and preview opened a connection to the bogus namespace
           'package:undefined' instead), which is exactly the failure mode
           spec 18's own notes warned a mismatched signature could cause. */
        previewHandle = P.preview(host, {
